...
"""
def test(feat_name='res5c_branch2b',
          exp_name='test_1',
          ...
          save_dir='exp',
          model_name = 'depth_of_field.npz'): # the directory to save the model, default is in exp
    ...
    if reload_:
        model_saved = os.path.join(from_dir, model_name)
    ...
    for caps in test:
        ...
        if True:
            ...
            for jj in xrange(len(caps)):
                sample, score = gen_sample(tparams, f_init, f_next, 
                                           ctx_res5b_s[jj], ctx_res5c_branch2b_s[jj], model_options,
                                           trng=trng, k=5, maxlen=100, stochastic=False)
