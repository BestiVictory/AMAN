...
def pred_probs(f_log_probs, options, worddict, prepare_data, data, iterator, verbose=False):
	...
	n_done = 0
	#for num1, valid_index1 in enumerate(iterator.split(data[0])):
	#	print(num1,valid_index1)
	for _, valid_index in enumerate(iterator.split(data[0])):
		x, mask, ctx_res5b, ctx_res5c_branch2b = prepare_data([data[0][t] for t in valid_index[1]],
															   data[1],
															   data[2],
															   worddict,
															   maxlen=None,
		...
		probs[valid_index[1]]= pred_probs[:,None]

		L[valid_index[1]] = mask.sum(0).reshape([mask.shape[1], 1])
...
#			model_options = pkl.load(f)
	...
	print(str(os.path.join(save_path, 'model.npz.pkl')))
...
#		history_score = numpy.load(os.path.join(from_dir, 'model_best_so_far.npz'))['history_score'].tolist()
	...
	for eidx in range(max_epochs):
		...
		if save_per_epoch:
			numpy.savez(save_path + 'epoch_' + str(eidx + 1), history_errs=history_errs, history_score=history_score, **unzip(tparams))
