...
# lezhang.thu
from data import ava_3

import utils
from utils.util import *

# lezhang.thu_start
from init_docfreq import CiderScorerAlias
...
# lezhang.thu
datasets = {'ava': (ava.load_data, ava.prepare_data),
			'ava_3': (ava_3.load_data, ava_3.prepare_data)}
...
def lstm_cond_layer(tparams, conv_params, bn_params, state_below, options, prefix='lstm',
	...
	if context_res5c_branch2b.ndim == 4:
		context_res5c_branch2b_reshaped = context_res5c_branch2b.reshape(
			[branch2b_shape[0], branch2b_shape[1], branch2b_shape[2] * branch2b_shape[3]]).transpose(0, 2, 1)
	elif context_res5c_branch2b.ndim == 3:
		context_res5c_branch2b_reshaped = context_res5c_branch2b.reshape(
			[branch2b_shape[0], branch2b_shape[1] * branch2b_shape[2]]).transpose(1, 0)
	else:
	...
	def _step(m_, x_, h_, c_, a_1, as_1, a_2, as_2, b_1, bs_1, b_2, bs_2, ct_, dp_=None, dp_att_=None):
		...
		pctx_1_ = tensor.dot(context_res5c_branch2b_reshaped_mean[:, :, None],
							 tparams[_p(prefix, 'Wc_att_res5c_branch2b_1')]) + tparams[
					  _p(prefix, 'b_att_res5c_branch2b_1')]
		pstate_1_ = tensor.dot(h_, tparams[_p(prefix, 'Wd_att_res5c_branch2b_1')])
		pctx_1_ = pctx_1_ + pstate_1_[:, None, :]
		pctx_1_ = tanh(pctx_1_)

		alpha_1 = tensor.dot(pctx_1_, tparams[_p(prefix, 'U_att_res5c_branch2b_1')]) + tparams[
			_p(prefix, 'c_att_res5c_branch2b_1')]
		...
		pctx_2_ = tensor.dot(weighted_context_res5c_branch2b, tparams[_p(prefix, 'Wc_att_res5c_branch2b_2')]) + tparams[
			_p(prefix, 'b_att_res5c_branch2b_2')]
		pstate_2_ = tensor.dot(h_, tparams[_p(prefix, 'Wd_att_res5c_branch2b_2')])
		pctx_2_ = pctx_2_ + pstate_2_[:, None, :]
		pctx_2_ = tanh(pctx_2_)

		alpha_2 = tensor.dot(pctx_2_, tparams[_p(prefix, 'U_att_res5c_branch2b_2')]) + tparams[
			_p(prefix, 'c_att_res5c_branch2b_2')]
		...
		context_res5c_reshaped = context_res5c.reshape(
			[res5c_shape[0], res5c_shape[1], res5c_shape[2] * res5c_shape[3]]).transpose(0, 2, 1)

		context_res5c_reshaped_mean = context_res5c_reshaped.mean(axis=1)
		pctx_3_ = tensor.dot(context_res5c_reshaped_mean[:, :, None], tparams[_p(prefix, 'Wc_att_res5c_1')]) + tparams[
			_p(prefix, 'b_att_res5c_1')]
		...
		pctx_4_ = tensor.dot(weighted_context_res5c, tparams[_p(prefix, 'Wc_att_res5c_2')]) + tparams[
			_p(prefix, 'b_att_res5c_2')]
		...
		rval = [h, c, alpha_1, alpha_1_sample, alpha_2, alpha_2_sample, beta_1, beta_1_sample, beta_2, beta_2_sample,
				ctx_]
		if options['selector']:
			rval += [sel_]
		rval += [i, f, o, preact]
	...
	if one_step:
		if options['use_dropout_lstm']:
			if options['selector']:
				rval = _step0(mask, state_below, dp_mask, init_state, init_memory, None, None, None, None, None, None,
							  None, None, None, None)
			else:
				rval = _step0(mask, state_below, dp_mask, init_state, init_memory, None, None, None, None, None, None,
							  None, None, None)
			...
			if options['selector']:
				rval = _step0(mask, state_below, init_state, init_memory, None, None, None, None, None, None, None,
							  None, None, None)
			else:
				rval = _step0(mask, state_below, init_state, init_memory, None, None, None, None, None, None, None,
							  None, None)
...
def init_params(options):
	...
	if options['n_layers_lstm'] > 1:
		for lidx in range(1, options['n_layers_lstm']):
			params = get_layer('ff')[0](options, params, prefix='ff_state_%d' % lidx, nin=options['ctx_dim'],
										nout=options['dim'])
			params = get_layer('ff')[0](options, params, prefix='ff_memory_%d' % lidx, nin=options['ctx_dim'],
										nout=options['dim'])
	...
	if options['n_layers_out'] > 1:
		for lidx in range(1, options['n_layers_out']):
			params = get_layer('ff')[0](options, params, prefix='ff_logit_h%d' % lidx, nin=options['dim_word'],
...
def build_model(tparams, conv_params, bn_params, options, sampling=True):
	...
	reward = tensor.matrix('reward', dtype='float32')  # lezhang.thu
	...
	reward_flat = reward.flatten()  # lezhang.thu
	# tensor.arange(x_flat.shape[0]) * probs.shape[1] get all the first index of each row in p_flat, x_flat is the offset
	# lezhang.thu_start
	cost = -reward_flat * tensor.log(
		p_flat[tensor.arange(x_flat.shape[0]) * probs.shape[1] + x_flat] + 1e-8
	)
	# lezhang.thu_end
	cost = cost.reshape([x.shape[0], x.shape[1]])
	masked_cost = cost * mask
	cost = masked_cost.sum(0)  # lezhang.thu
	...
	# lezhang.thu
	return trng, use_noise, [x, mask, reward, ctx_res5b,
							 ctx_res5c_branch2b], alphas_1, alpha_1_sample, alphas_2, alpha_2_sample, betas_1, beta_1_sample, betas_2, beta_2_sample, cost, opt_outs
...
def build_sampler(tparams, conv_params, bn_params, options, use_noise, trng, sampling=True):
	...
	if options['n_layers_lstm'] > 1:
		for lidx in range(1, options['n_layers_lstm']):
			...
			init_memory.append(
				get_layer('ff')[1](tparams, ctx_mean, options, prefix='ff_memory_%d' % lidx, activ='tanh'))

	print('Building f_init...')
	f_init = theano.function([ctx_res5b, ctx_res5c_branch2b],
							 [ctx_res5b, ctx_res5c_branch2b] + init_state + init_memory, name='f_init', profile=False)
	print('Done')
	...
	f_next = theano.function([x, ctx_res5b, ctx_res5c_branch2b] + init_state + init_memory,
							 [next_probs, next_sample] + next_state + next_memory, name='f_next', profile=False)

	return f_init, f_next


# build a sampler
def build_sampler_greedy(tparams, conv_params, bn_params, options, use_noise, trng, sampling=True):
	""" Builds a sampler used for generating from the model
	Parameters
	----------
		See build_model function above
	...
	# lezhang.thu
	next_sample = next_probs.argmax(1)  # greedy
...
# build a sampler
def build_sampler_uniform(tparams, conv_params, bn_params, options, use_noise, trng, sampling=True):
	""" Builds a sampler used for generating from the model
	Parameters
	----------
		See build_model function above
	...
	logit_shp = logit.shape
	next_probs = tensor.nnet.softmax(logit)
	next_sample = trng.multinomial(pvals=next_probs).argmax(1)  # uniform
...
def pred_probs(f_log_probs, options, worddict, prepare_data, data, iterator, verbose=False):
	...
	for _, valid_index in iterator:
		# lezhang.thu
		_, x, mask, ctx_res5b, ctx_res5c_branch2b = prepare_data([data[0][t] for t in valid_index],
																 data[1],
																 data[2],
																 worddict,
																 maxlen=None,
		...
		# lezhang.thu
		reward = numpy.ones((mask.shape[0], mask.shape[1]), dtype=numpy.float32)
		pred_probs = f_log_probs(x, mask, reward, ctx_res5b, ctx_res5c_branch2b)
...
def train(feat_name='res5c_branch2b',
		  ...
		  save_per_epoch=True,  # this saves down the model every epoch
	...
	# lezhang.thu_start
	scorer_alias = CiderScorerAlias(train)
	# lezhang.thu_end
	...
	with open(os.path.join(save_path, 'model.npz.pkl'), "wb") as f:
	...
	if finetune_conv_params:
		if reload_:
			...
			for kk, vv in conv_params.items():
				if kk in pp:
	...
	# lezhang.thu_start
	f_init_uniform, f_next_uniform = build_sampler_uniform(
		tparams, conv_params, bn_params, model_options, use_noise, trng
	)
	f_init_greedy, f_next_greedy = build_sampler_greedy(
		tparams, conv_params, bn_params, model_options, use_noise, trng
	)
	# lezhang.thu_end
	...
	if valid:
		kf_valid = KFold(n_splits=len(valid[0])//valid_batch_size, shuffle=False)
	if test:
		kf_test = KFold(n_splits=len(test[0])//valid_batch_size, shuffle=False)
	...
	for eidx in range(max_epochs):
		...
		for caps in train_iter:
			...
			pd_start = time.time()
			# lezhang.thu
			x_id, x, mask, ctx_res5b, ctx_res5c_branch2b = prepare_data(caps,
																		train[1],
																		train[2],
																		worddict,
																		maxlen=maxlen,
			...
			xx_maxlen, xx_batch_size = xx.shape
			# debug
			baseline = [None] * xx_batch_size

			use_noise.set_value(0.)
			reward = [None] * xx_batch_size
			for kk in range(xx_batch_size):
				uniform, _ = gen_sample(
					tparams,
					f_init_uniform,
					f_next_uniform,
					ctx_res5b[kk],
					ctx_res5c_branch2b[kk],
					model_options,
					trng=trng,
					maxlen=maxlen,
					stochastic=True
				...
				# debug
				# greedy, greedy_score = gen_sample(tparams, f_init, f_next, ctx_res5b[kk],
				#								   ctx_res5c_branch2b[kk], model_options, trng=trng,
				#								   k=5, maxlen=maxlen, stochastic=False)
				# # greedy = greedy[np.argmin(greedy_score)]
				# greedy = greedy[0]

				iter_num = 20
				avg = 0.0
				for _ in range(iter_num):
					avg_sample, _ = gen_sample(
						tparams,
						f_init_uniform,
						f_next_uniform,
						ctx_res5b[kk],
					...
					avg += scorer_alias.compute_score(x_id[kk], avg_sample, word_idict)
				avg /= iter_num
				# greedy, _ = gen_sample(
				#	 tparams,
				#	 f_init_greedy,
				...
				# )
				# calculating reward
				reward[kk] = scorer_alias.compute_score(x_id[kk], uniform, word_idict)
				# debug
				baseline[kk] = avg
				reward[kk] -= baseline[kk]
				# reward[kk] -= scorer_alias.compute_score(x_id[kk], greedy, word_idict)

				x[:len(uniform), kk] = uniform
				x[len(uniform):, kk] = 0
				mask[:len(uniform), kk] = 1
				mask[len(uniform):, kk] = 0
			use_noise.set_value(1.)

			baseline = np.array(baseline).astype(np.float32)
			reward = np.array(reward).astype(np.float32)

			# debug
			print('uidx {}, adv[:10]: {}'.format(uidx, reward[:10]))
			print('uidx {}, baseline[:10]: {}\n'.format(uidx, baseline[:10]))

			reward = np.repeat(reward[np.newaxis, :], xx_maxlen, axis=0)
			# get the cost for the minibatch, and update the weights
			ud_start = time.time()
			# cost = f_grad_shared(x, mask, ctx_res5b, ctx_res5c_branch2b)
			cost = f_grad_shared(x, mask, reward, ctx_res5b, ctx_res5c_branch2b)
			...
			if sampleFreq > 0 and numpy.mod(uidx, sampleFreq) == 0:
				...
				for jj in range(numpy.minimum(10, len(caps))):
					sample, score = gen_sample(tparams, f_init, f_next,
											   ...
											   trng=trng, k=5, maxlen=maxlen, stochastic=False)
			...
			if validFreq > 0 and numpy.mod(uidx, validFreq) == 0:
				...
				if valid_perp < 50:
					...
					if len(numpy.array(history_score)) > 1 and (
							valid_B4 > numpy.array(history_score)[:-1, 5].max() or valid_meteor > numpy.array(
						history_score)[:-1, 8].max() or valid_Cider > numpy.array(history_score)[:-1, 7].max()):
						...
						numpy.savez(os.path.join(save_path, "model_best_bleu.npz"), history_errs=history_errs,
									history_score=history_score, **params_best_bleu)

					if len(numpy.array(history_score)) > 1 and valid_err < numpy.array(history_score)[:-1, 16].min():
						best_p = unzip(tparams)
						print('Saving best model so far')
						params = copy.copy(best_p)
						numpy.savez(os.path.join(save_path, "model_best_so_far.npz"), history_errs=history_errs,
									history_score=history_score, **params)
						bad_counter = 0
		...
		if save_per_epoch:
			numpy.savez(save_path + 'epoch_' + str(eidx + 1), history_errs=history_errs, history_score=history_score,
						**unzip(tparams))
