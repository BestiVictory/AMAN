...
def generate_sample(valid, test, word_idict, f_init, f_next, options, beam, save_result_dir):
  ...
  def sample(split):
    ...
    for i in range(split[1].shape[0]):
      ...
      sample, sample_score = sca_resnet_branch2b_3.gen_sample(None, f_init, f_next, ctx_res5b, ctx_res5c_branch2b, options, None, beam, maxlen=MAXLEN)
